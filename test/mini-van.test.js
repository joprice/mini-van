...
const runTests = (van, msgDom) => {
    ...
    const tests = {
        ...
        dummyReactive: () => {
            ...
            }, () => state1.val, () => state2.oldVal, state3, () => state4.val), button({ onclick: van.derive(() => state5.val ? 'console.log("Hello")' : 'alert("Hello")') }, "Button1"), button({
                onclick: van.derive(() => state6.val ? () => console.log("Hello") : () => alert("Hello"))
            }, "Button2"), () => (state5.val ? pre : div)(state3), () => (state6.oldVal ? pre : div)(state4));
        ...
        html: () => {
            assertEq(van.html(head(title("Hello")), body(div("World"))), "<!DOCTYPE html><html><head><title>Hello</title></head><body><div>World</div></body></html>");
            assertEq(van.html({ lang: "en" }, head(title("Hello")), body(div("World"))), '<!DOCTYPE html><html lang="en"><head><title>Hello</title></head><body><div>World</div></body></html>');
        },
    };
    ...
    for (const [k, v] of Object.entries(suites)) {
        for (const [name, func] of Object.entries(v)) {
            ...
            van.add(msgDom, div(pre(`Running ${k}.${name}...`), resultPre, pre(" "), buttonDom, pre(" "), msgPre));
            try {
                func();
                resultPre.innerText = "✅";
            }
